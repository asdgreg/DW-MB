...
app.get("/ab*cd",function(peticion, respuesta){
	...
	conexion.query("select id_usuario, nombre_usuario, contrasena, correo from tbl_usuarios", function(err,filas,campos){

		var resultado = "";

		for (var i = 0; i < filas.length; i++) {
			resultado = "nombre: "+ filas[i].nombre_usuario + " contrasena: "+filas[i].contrasena +"<br>";
		}
...
app.post("/user-proy",urlEncodeParser,function(peticion, respuesta){
	console.log(peticion.body);
		conexion.query("SELECT * FROM tbl_proyectos where id_usuario_creador = ? ",
			[peticion.body.usuario],
			function(err, filas, campos){
				respuesta.send(JSON.stringify(filas));
			}
